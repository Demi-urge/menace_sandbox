...
if __name__ == "__main__" and len(sys.argv) > 1 and sys.argv[1] == "mvp-workflow-smoke":
    ...
    scoring_path = Path(__file__).with_name("sandbox_runner") / "scoring.py"
    ...
    if scoring_spec is None or scoring_spec.loader is None:
        raise SystemExit("Unable to load sandbox_runner.scoring for smoke workflow")
    scoring_module = importlib.util.module_from_spec(scoring_spec)
    scoring_spec.loader.exec_module(scoring_module)
    record_run = scoring_module.record_run
    load_summary = scoring_module.load_summary
    ...
    workflow_entropy = _compute_workflow_entropy([{"module": mod} for mod in module_list])
    last_entropy = 0.0
    ...
    if not pipeline_invocations:
        raise SystemExit("MVP brain was not invoked during smoke workflow")
    if not patch_proposed:
        raise SystemExit("MVP brain did not propose any patches")
    if not validation_blocked:
        raise SystemExit("Patch validation did not block any invalid patches")
    if not stabilized_steps:
        raise SystemExit("No workflow steps stabilized after patching")
    if run_delta <= 0 or entropy_delta_total == 0.0:
        raise SystemExit("Workflow metrics failed to update in run summary")
    if roi_count_delta <= 0:
        raise SystemExit("ROI metrics failed to update in run summary")
    raise SystemExit(0)
