...
class ForesightTracker:
    ...
    def record_cycle_metrics(self, workflow_id: str, metrics: Dict[str, float]) -> None:
        ...
        metrics:
            Dictionary containing ``roi_deltas``, ``raroi_deltas``,
    ...
    def get_trend_curve(self, workflow_id: str) -> Tuple[float, float, float]:
        """Return slope, second derivative and average stability for ROI deltas."""

        data = self.history.get(workflow_id)
        if not data or len(data) < 2:
            return 0.0, 0.0, 0.0

        roi = [entry["roi_deltas"] for entry in data]
        n = len(roi)
        x = list(range(n))
        ...
        avg_stability = statistics.mean(abs(d) for d in first_diff) if first_diff else 0.0
    ...
    def is_stable(self, workflow_id: str, volatility_threshold: float = 1.0) -> bool:
        ...
        deviation = statistics.stdev(roi) if len(roi) > 1 else 0.0
        return slope > 0 and deviation < volatility_threshold
