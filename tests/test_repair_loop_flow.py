...
def test_run_repair_loop_changes_working_directory(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    subprocess.run(["git", "init"], cwd=repo, check=True, capture_output=True)
    ...
    calls: dict[str, dict[str, object]] = {}
    cwd_trace: dict[str, str] = {}

    def fake_validate(**kwargs: object) -> tuple[bool, list[str]]:
        cwd_trace.setdefault("validate", os.getcwd())
        calls["validate"] = kwargs
        return True, []

    def fake_apply(**kwargs: object) -> tuple[bool, int | None, list[str]]:
        cwd_trace.setdefault("apply", os.getcwd())
        calls["apply"] = kwargs
        return True, 1, []
    ...
    assert outcome.fail_count == 0
    assert Path(calls["validate"]["module_path"]).resolve() == target.resolve()
    assert cwd_trace["validate"] == str(repo.resolve())
    assert cwd_trace["apply"] == str(repo.resolve())
