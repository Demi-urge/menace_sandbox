...
def test_classify_by_exception_type():
    ...
    stack = "Traceback...\nKeyError: missing"
    assert classify_exception(err, stack) is LegacyErrorCategory.RuntimeFault


def test_extended_exception_types():
    assert (
        classify_exception(ZeroDivisionError("bad divide"), "")
        is LegacyErrorCategory.RuntimeFault
    )
    assert (
        classify_exception(AttributeError("no attr"), "")
        is LegacyErrorCategory.RuntimeFault
    )
    assert classify_exception(OSError("lib"), "") is LegacyErrorCategory.DependencyMismatch


def test_permission_http_and_sandbox_exception_types():
    assert (
        classify_exception(PermissionError("no access"), "")
        is LegacyErrorCategory.RuntimeFault
    )

    http_err = HTTPError("http://x", 404, "not found", hdrs=None, fp=None)
    assert classify_exception(http_err, "") is LegacyErrorCategory.ExternalAPI

    sbe = SandboxRecoveryError("boom")
    assert classify_exception(sbe, "") is LegacyErrorCategory.RuntimeFault


def test_classify_by_keyword():
    err = Exception("boom")
    stack = "operation failed due to dependency missing foo"
    assert classify_exception(err, stack) is LegacyErrorCategory.DependencyMismatch
...
def test_keyword_new_categories():
    ...
    assert (
        ...
        is LegacyErrorCategory.ResourceLimit
    )
    assert (
        classify_exception(err, "request timed out after 5s")
        is LegacyErrorCategory.Timeout
    )
    assert (
        classify_exception(err, "external api returned 500")
        is LegacyErrorCategory.ExternalAPI
    )
...
def test_extended_keyword_and_module():
    ...
    assert classify_exception(err2, "") is LegacyErrorCategory.DependencyMismatch
...
def test_classify_by_module():
    ...
    assert classify_exception(err, "") is LegacyErrorCategory.DependencyMismatch


def test_new_categories_by_type_and_module():
    assert (
        classify_exception(MemoryError("oom"), "") is LegacyErrorCategory.ResourceLimit
    )
    assert classify_exception(TimeoutError("slow"), "") is LegacyErrorCategory.Timeout

    class RequestsError(Exception):
        __module__ = "requests"

    err = RequestsError("bad response")
    assert classify_exception(err, "") is LegacyErrorCategory.ExternalAPI


def test_classify_unknown():
    err = Exception("something")
    assert classify_exception(err, "") is LegacyErrorCategory.Unknown
