...
def test_dependency_broker_advertisement_short_circuits(monkeypatch):
    """An advertised broker pipeline prevents redundant bootstrap work."""

    broker_manager = mock.Mock(name="broker_manager")
    broker_pipeline = SimpleNamespace(manager=broker_manager)
    broker_promote = mock.Mock(name="broker_promote")
    ...
    broker = mock.Mock(
        resolve=mock.Mock(return_value=(broker_pipeline, broker_manager)),
        advertise=mock.Mock(),
    )
    module._BOOTSTRAP_STATE = SimpleNamespace(
        helper_promotion_callbacks=[broker_promote], depth=1
    )
    monkeypatch.setattr(
        module, "_bootstrap_dependency_broker", mock.Mock(return_value=broker)
    )
    ...
    monkeypatch.setattr(module, "_looks_like_pipeline_candidate", lambda value: bool(value))
    monkeypatch.setattr(module, "_using_bootstrap_sentinel", lambda *_a, **_k: False)
    ...
    broker.advertise.assert_called_once_with(
        pipeline=broker_pipeline, sentinel=broker_manager
    )
    broker_promote.assert_called_once_with(broker_manager)
    module.prepare_pipeline_for_bootstrap.assert_not_called()
...
def test_bootstrap_depth_blocks_new_pipeline(monkeypatch):
    ...
    assert getattr(state.pipeline, "bootstrap_placeholder", False)
    module.prepare_pipeline_for_bootstrap.assert_not_called()
...
def test_dependency_resolution_caps_with_snapshot(monkeypatch):
    ...
    module.read_bootstrap_heartbeat = lambda: None
    module._resolve_bootstrap_wait_timeout = lambda *_a, **_k: None
    module._resolve_dependency_resolution_timeout = lambda *_a, **_k: 1.0
    module._resolve_dependency_max_wait = lambda default, cap=None: min(default, 0.02)
    ...
    assert "Broker snapshot" in str(excinfo.value)
    assert counter["now"] < 0.5
    module.prepare_pipeline_for_bootstrap.assert_not_called()
...
def test_active_heartbeat_uses_broker_pipeline(monkeypatch):
    ...
    module._BOOTSTRAP_STATE = SimpleNamespace(helper_promotion_callbacks=[broker_promote])
    module._bootstrap_dependency_broker = lambda: SimpleNamespace(
        resolve=lambda: (broker_pipeline, broker_manager)
    )
    module._current_bootstrap_context = lambda: None
    module._peek_owner_promise = lambda *_a, **_k: None
    module._using_bootstrap_sentinel = lambda *_a, **_k: False
...
def test_delayed_broker_advertisement_is_reused(monkeypatch):
    """Modules wait for broker advertisement instead of rebuilding during bootstrap."""
    ...
    module._BOOTSTRAP_STATE = SimpleNamespace(
        helper_promotion_callbacks=[broker_promote], depth=1, active_bootstrap_guard=None
    )
    module._bootstrap_dependency_broker = lambda: SimpleNamespace(resolve=_delayed_resolve)
    module._current_bootstrap_context = lambda: None
    module._peek_owner_promise = lambda *_a, **_k: None
    ...
    module._using_bootstrap_sentinel = lambda *_a, **_k: False
    module.read_bootstrap_heartbeat = lambda: {"ts": time.time()}
    module._resolve_bootstrap_wait_timeout = lambda *_a, **_k: 0.2
...
def test_active_heartbeat_returns_placeholder(monkeypatch):
    ...
    module._BOOTSTRAP_STATE = SimpleNamespace(helper_promotion_callbacks=[broker_promote])
    module._bootstrap_dependency_broker = lambda: SimpleNamespace(
        resolve=lambda: (None, broker_manager)
    )
    module._current_bootstrap_context = lambda: None
    module._peek_owner_promise = lambda *_a, **_k: None
    module._using_bootstrap_sentinel = lambda *_a, **_k: False
