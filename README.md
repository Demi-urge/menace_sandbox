...
Set `SANDBOX_DEBUG=1` to automatically enable debug logs when
`setup_logging()` is called without a level. The older `SANDBOX_VERBOSE=1`
variable is still accepted for backward compatibility.
