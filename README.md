...
All bootstrap participants now funnel through a centralised gate so only one bootstrap chain runs at a time. Call `bootstrap_gate.wait_for_bootstrap_gate()` (or `resolve_bootstrap_placeholders()` when you also need the active pipeline, manager, and broker) before invoking `prepare_pipeline_for_bootstrap` so new helpers reuse the advertised placeholders instead of racing the single-flight coordinator. A short overview of the single-entry bootstrap contract and readiness dependency expectations lives in [docs/bootstrap.md](docs/bootstrap.md) for developers wiring new services into the gate. Entry scripts should follow the pattern:

1. `advertise_bootstrap_placeholder()` as soon as imports begin so late arrivals reuse the sentinel manager.
2. `bootstrap_gate.wait_for_bootstrap_gate()` to align with the shared queue and respect host-level backoff.
3. `prepare_pipeline_for_bootstrap(..., bootstrap_guard=True)` to claim the single-flight slot and reuse the gate placeholders.

Direct calls into `_bootstrap_manager`, `bootstrap_self_coding.bootstrap()`, or bespoke `ModelAutomationPipeline` constructors that bypass the gate are now forbidden; they can deadlock the shared coordinator and will be treated as regression bugs. Keep new bootstrap entrypoints on the three-step path above so their heartbeats, readiness signals, and sentinels flow through the central gate.
