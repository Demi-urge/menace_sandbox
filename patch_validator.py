"""Deterministic patch validation utilities."""
...
from menace.errors import PatchRuleError, PatchSyntaxError, PatchValidationError
    ...
    *,
    ...
    try:
        ...
        error = PatchValidationError(
            "unexpected patch text validation error",
            details={
                "exception_type": exc.__class__.__name__,
                "exception_message": str(exc),
                "stage": "validate_patch_text",
            },
        )
        return {
            "valid": False,
            "flags": ["patch_validation_error"],
            "context": {"error": error.to_dict()},
        }
...
def _validate_patch_text(
    ...
    *,
    ...
    if not isinstance(patch_text, str):
        return {"valid": False, "flags": ["patch_not_string"], "context": {}}

    if not patch_text.strip():
        return {"valid": False, "flags": ["patch_empty"], "context": {}}
    ...
    return {"valid": not bool(flags), "flags": flags, "context": context}


def _iter_disallowed_lines(lines: Sequence[str]) -> list[str]:
    flags: list[str] = []
    for line in lines:
...
def validate_patch(
    ...
    *,
    ...
    schema_errors: list[PatchValidationError] = []
    ...
    try:
        ...
        if not schema_errors:
            validated_rules, schema_errors = _validate_rule_schema(rules_list)
        ...
        if isinstance(original, str) and isinstance(patched, str) and not schema_errors:
            ...
            for validated_rule in validated_rules:
                ...
                if rule_type in {"syntax", "syntax_compile"}:
                    failures, summary = _apply_syntax_compile(
                        params,
                        syntax_errors,
                    ...
                    failures, summary = _dispatch_rule(
                        rule_type,
                        params,
                        rule_index=rule_index,
                        rule_id=validated_rule.rule_id,
...
def _normalize_rules(
    rules: Sequence[Mapping[str, object]] | Mapping[str, object]
) -> tuple[list[Mapping[str, object]], PatchValidationError | None]:
...
def _validate_rule_schema(
...
) -> tuple[list[ValidatedRule], list[PatchValidationError]]:
    ...
    for rule_index, rule in enumerate(rules):
        ...
        raw_rule_id = rule.get("id", rule.get("rule_id"))
...
def _validate_syntax_compile_params(
    ...
    *,
    ...
    _validate_require_changes_param(params, rule_index=rule_index, rule_id=rule_id, errors=errors)
...
def _validate_signature_match_params(
    ...
    *,
    ...
    _validate_require_changes_param(normalized, rule_index=rule_index, rule_id=rule_id, errors=errors)
...
def _validate_forbidden_patterns_params(
    ...
    *,
    ...
    if not any(normalized.get(key) for key in list_fields):
        errors.append(
            _schema_error(
...
def _dispatch_rule(
    ...
    *,
    ...
    if rule_type == "required_imports":
        summary["data"] = _apply_required_imports(
            params, patched_tree, rule_errors, rule_index=rule_index
        )
    elif rule_type == "signature_match":
        ...
        summary["data"] = _apply_forbidden_patterns(
            params, patched_tree, rule_errors, rule_index=rule_index
        )
    elif rule_type == "static_contracts":
        summary["data"] = _apply_static_contracts(
            params, patched_nodes, rule_errors, rule_index=rule_index
        )
    elif rule_type == "mandatory_returns":
        summary["data"] = _apply_mandatory_returns(
            params, patched_nodes, rule_errors, rule_index=rule_index
        )
    else:
        rule_errors.append(
...
def _apply_syntax_compile(
    ...
    *,
    ...
    rule_errors: list[PatchRuleError] = []
    sources = params.get("sources") or params.get("source") or "patched"
...
def _apply_signature_matching(
    ...
    *,
    ...
    functions = params.get("functions") or params.get("function")
    classes = params.get("classes") or params.get("class")
    if isinstance(functions, str):
        functions = [functions]
    if isinstance(classes, str):
        classes = [classes]
    require_annotations = bool(params.get("require_annotations"))
    ...
    for name in classes:
        ...
        if original_sig is None or patched_sig is None:
            ...
            continue
        original_payload = _signature_payload(
            original_sig["signature"], include_annotations=require_annotations
        )
        patched_payload = _signature_payload(
            patched_sig["signature"], include_annotations=require_annotations
        )
        if original_payload != patched_payload:
            mismatch = {
                "symbol": name,
...
def _apply_forbidden_patterns(
    ...
    *,
    ...
    node_types = params.get("node_types") or params.get("types") or []
    names = params.get("names") or []
    strings = params.get("strings") or []
    call_names = params.get("call_names") or []
    attributes = params.get("attributes") or []
    name_patterns = params.get("name_patterns") or []
    string_patterns = params.get("string_patterns") or []
    call_patterns = params.get("call_patterns") or []
    attribute_patterns = params.get("attribute_patterns") or []
...
def _apply_mandatory_returns(
    ...
    *,
    ...
    if isinstance(functions, str):
        functions = [functions]
    require_terminal = bool(params.get("require_terminal"))
    require_any = params.get("require_any", True)
...
def _apply_static_contracts(
    ...
    *,
    ...
    if isinstance(functions, str):
        functions = [functions]
    require_docstring = bool(params.get("require_docstring"))
    require_annotations = bool(params.get("require_annotations"))
    must_raise = params.get("must_raise") or []
    if not isinstance(functions, list) or not functions:
        rule_errors.append(
            PatchRuleError(
...
def _apply_change_requirement(
    ...
    *,
    ...
    if not params.get("require_changes"):
        return None
    functions = params.get("functions") or params.get("function")
    classes = params.get("classes") or params.get("class")
    if isinstance(functions, str):
