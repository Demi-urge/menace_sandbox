"""Fine-tune a small language model to predict new tools for bots.

The training data should be a JSONL file where each line contains the fields
``bot_description`` and ``added_tool`` as produced by
:mod:`micro_models.tool_history_dataset`.
"""
...
def _prep_dataset(tokenizer, path: Path):
    ...
    def _format(example):
        prompt = (
            "Given the following bot description, predict the next tool to add.\n"
            + example["bot_description"]
            + "\nTool:"
        )
        input_ids = tokenizer(prompt).input_ids  # type: ignore[union-attr]
        labels = tokenizer(example["added_tool"]).input_ids  # type: ignore[union-attr]
...
def train(dataset_path: Path, output_dir: Path, model_name: str = DEFAULT_MODEL) -> None:
    """Fine-tune ``model_name`` on ``dataset_path`` and write to ``output_dir``."""
...
def main() -> None:  # pragma: no cover - CLI utility
    parser = argparse.ArgumentParser(description="Fine-tune tool predictor")
