...
optional FAISS or Annoy based vector search. If neither backend is available an in-memory linear search is used.
"""
...
class EmbeddableDBMixin:
    ...
    def add_embedding(
        ...
        """Store ``vector`` for ``id`` and add it to the search index."""
        ...
        self.conn.execute(
            ...
            """,
            (str(id), vec_json, created_at, version, kind, source_id),
        )
        self.conn.commit()
    ...
    def search_by_vector(
        self, query_vec: Sequence[float], limit: int = 5
    ) -> List[Tuple[Any, float]]:
        """Search embeddings similar to ``query_vec``.
        ...
        if self._vector_index is None:
            self._load_index()
        if self._vector_index is None:
            cur = self.conn.execute("SELECT record_id, vector FROM embeddings")
            rows = cur.fetchall()
            results: List[Tuple[Any, float]] = []
            for rid, vec_json in rows:
        ...
        ids, dists = self._vector_index.get_nns_by_vector(  # type: ignore
            list(query_vec), limit, include_distances=True
        )
        return [
            (self._id_map[i], float(d))
