...
class NetworkedEventBus:
    ...
    def _ensure_consumer(self, topic: str) -> None:
        ...
        if self._channel:
            def _consume() -> None:
                conn = pika.BlockingConnection(pika.ConnectionParameters(host=self._host))
                channel = conn.channel()
                channel.queue_declare(queue=topic, durable=True)
                for method_frame, properties, body in channel.consume(queue=topic, inactivity_timeout=1):
                    ...
                    for cb in self._subs.get(topic, []):
                        try:
                            ...
                            logging.getLogger(__name__).error("subscriber failed: %s", exc)
                    for acb in self._async_subs.get(topic, []):
                        try:
                            ...
                            logging.getLogger(__name__).error("async subscriber failed: %s", exc)
                    channel.basic_ack(method_frame.delivery_tag)
                try:
                    channel.close()
                except Exception as exc:
                    logging.getLogger(__name__).error("consumer channel close failed: %s", exc)
                try:
                    conn.close()
                except Exception as exc:
                    logging.getLogger(__name__).error("consumer connection close failed: %s", exc)
        else:
