"""Fine-tune a tiny model to categorise stack traces and suggest fixes.

The expected training data is a JSONL file where each row contains
``stack_trace``, ``cause`` and ``error_type`` fields as produced by
:mod:`micro_models.error_dataset`.
"""
...
def _prep_dataset(tokenizer, path: Path):
    ...
    def _format(example):
        ...
        target = f"Category: {example['error_type']}\nFix: {example['cause']}"
        input_ids = tokenizer(prompt).input_ids  # type: ignore[union-attr]
        labels = tokenizer(target).input_ids  # type: ignore[union-attr]
...
def main() -> None:  # pragma: no cover - CLI utility
    parser = argparse.ArgumentParser(description="Fine-tune error classifier")
