...
import menace.evolution_predictor as ep
...
def test_train_predict(tmp_path, monkeypatch):
    ...
    bot = ep.EvolutionPredictor(hist, data_bot=data_bot, capital_bot=cap_bot)
    ...
    monkeypatch.setattr(ep.DataBot, "complexity_score", staticmethod(lambda df: 1.0))
    ...
    monkeypatch.setattr(ep.DataBot, "complexity_score", staticmethod(lambda df: 2.0))
    monkeypatch.setattr(cap_bot, "bot_roi", lambda a: 2.0)
    pred1 = bot.predict("a", 1.0)

    monkeypatch.setattr(ep.DataBot, "complexity_score", staticmethod(lambda df: 4.0))
