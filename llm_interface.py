...
class OpenAIProvider(LLMClient):
    ...
    def __init__(
        ...
        *,
        ...
        if base_url:
            ...
            elif base_url.endswith("/v1/responses") or base_url.endswith("/responses"):
                self.api_url = base_url
        ...
        if os.getenv("OPENAI_USE_RESPONSES"):
            self._use_responses_api = os.getenv("OPENAI_USE_RESPONSES", "").lower() not in {
                "0",
                "false",
                "no",
            }
        elif "/responses" in self.api_url:
    ...
    def _prepare_payload(self, prompt: Prompt) -> Dict[str, Any]:
        ...
        if self._use_responses_api:
            payload: Dict[str, Any] = {"model": self.model, "input": messages}
        else:
    ...
    def _generate(
        ...
        if self._use_responses_api:
            ...
            if isinstance(input_payload, list):
                ...
                prompt_text = str(input_payload)
        else:
            prompt_text = " ".join(m.get("content", "") for m in payload["messages"])
        prompt_tokens_est = rate_limit.estimate_tokens(prompt_text, model=self.model)
    ...
    def generate(
        ...
        *,
        ...
        if self._use_responses_api:
            ...
            prompt_text = " ".join(m.get("content", "") for m in payload["messages"])
        prompt_tokens = rate_limit.estimate_tokens(prompt_text, model=self.model)
        cfg = llm_config.get_config()
        self._rate_limiter.update_rate(cfg.tokens_per_minute)
