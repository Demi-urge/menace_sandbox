...
def _mark_deferred_gates(
    *,
    ...
    remaining_window: float = 0.0,
) -> Mapping[str, Any]:
    """Record a degraded-but-online state for optional gates."""
...
def _extend_component_windows(
    ...
    *,
    ...
    readiness_pending: Iterable[str],
    vector_heavy: bool,
    backoff_cap: float = _GATE_EXTENSION_BACKOFF_CAP,
    extension_limit: int = _GATE_EXTENSION_LIMIT,
    time_fn: Callable[[], float] = time.perf_counter,
) -> tuple[list[str], list[str], list[dict[str, Any]]]:
    """Extend component windows when forward progress is detected."""
    ...
    for gate, window in gate_windows.items():
        ...
        if gate_progressing and extensions < extension_limit:
            ...
            window["extensions"] = extensions + 1
...
def _get_bootstrap_wait_timeout() -> float | None:
    ...
    try:
        ...
        component_budget_source = "persisted"
        component_budgets = {}
    if component_budgets:
        budget_pools.update(component_budgets)
        component_budget_source = "computed"
    ...
    if component_budget_total is None and budget_pools:
        component_budget_total = sum(
            value for key, value in budget_pools.items() if key not in DEFERRED_COMPONENTS
        )
    ...
    background_unlimited = _truthy_env(os.getenv(_BACKGROUND_UNLIMITED_ENV))
    if not raw_timeout:
    ...
    if requested_timeout is not None:
        ...
        base_timeout = max(base_timeout, clamped_timeout)
        requested_cap = clamped_timeout
    ...
    adaptive_timeout = min(
        base_timeout * contention_scale,
        base_timeout * (1.0 + _BOOTSTRAP_WAIT_GROWTH_CAP),
    )

    per_component_deadlines: dict[str, float | None] = {}
    for component, pool in budget_pools.items():
        scaled_pool = pool * contention_scale if pool else 0.0
        deadline = max(pool, scaled_pool)
        if component in DEFERRED_COMPONENTS and background_unlimited:
            deadline = None
        per_component_deadlines[component] = deadline

    max_component_deadline = max(
        (value for value in per_component_deadlines.values() if value is not None),
        default=0.0,
    )
...
def _bootstrap_dependency_broker() -> _BootstrapDependencyBroker:
    ...
    try:
        broker.last_requester = caller  # type: ignore[attr-defined]
    except Exception:  # pragma: no cover - metadata attachment best effort
        logger.debug("failed to capture broker requester", exc_info=True)
    return broker
    ...
    while pending:
        ...
        if nodes_seen > max_nodes:
            logger.warning(
                ...
                max_depth,
                len(pending),
                "bootstrap_placeholder_propagation_limit",
            )
            return
...
def prepare_pipeline_for_bootstrap(
    ...
    **pipeline_kwargs: Any,
    ...
    priority_bootstrap_inflight = caller_matches_priority and (
        active_depth > 0 or broker_placeholder_active
    )
    if priority_bootstrap_inflight and not placeholder_needs_owner:
        telemetry = {
            "event": "prepare-pipeline-bootstrap-priority-recursion-guard",
            ...
            "priority_caller": True,
        }
        if broker_placeholder_without_owner:
            logger.error(
                "prepare_pipeline.bootstrap.priority_placeholder_block",
                extra=telemetry,
            )
            raise RuntimeError(
        ...
        if active_promise is not None:
            ...
            logger.info(
                "prepare_pipeline.bootstrap.priority_promise_guard_reuse",
                extra=telemetry,
            )
        ...
        if broker_placeholder_active and (broker_pipeline is not None or broker_sentinel is not None):
            dependency_broker.advertise(
                ...
                owner=broker_owner_active or True,
            )
        logger.info(
            "prepare_pipeline.bootstrap.priority_recursion_guard_short_circuit",
            extra={
                **telemetry,
    ...
    if caller_matches_priority and (active_depth > 0 or broker_placeholder_active):
        ...
        if active_promise is not None:
            ...
            telemetry["active_promise"] = True
            logger.info(
                "prepare_pipeline.bootstrap.priority_promise_short_circuit",
                extra=telemetry,
            )
            return active_promise.wait()
        ...
        logger.info(
            "prepare_pipeline.bootstrap.priority_broker_short_circuit",
            extra={
                **telemetry,
                "pipeline_candidate": getattr(
                    getattr(broker_pipeline, "__class__", None),
...
def _prepare_pipeline_for_bootstrap_impl_inner(
    ...
    **pipeline_kwargs: Any,
    ...
    def _check_stop_or_timeout(stage: str) -> None:
        ...
        watchdog_telemetry["adaptive_timeout_context"] = get_adaptive_timeout_context()
        ...
        if gate_windows:
            ...
            watchdog_telemetry["component_windows"] = gate_windows
        ...
        if gate_windows:
            ...
            if extended and not exhausted_gates:
                return
        ...
        for gate in evaluation_gates:
            ...
            if phase_state is None:
                phase_state = {
                    "budget": gate_budget,
                    "remaining": gate_budget,
                    "last_check": time.perf_counter(),
                    "pending": len(readiness_pending),
                    "ratio": readiness_ratio,
                    "grace": False,
                }
                if window and window.get("deadline") is not None:
                    phase_state["deadline"] = window.get("deadline")
            ...
            pending_delta = phase_state.get("pending", 0) - len(readiness_pending)
            ratio_delta = readiness_ratio - phase_state.get("ratio", 0.0)
            progressing = pending_delta > 0 or ratio_delta > 0 or staged_ready_signal
            if progressing and not phase_state.get("grace"):
                grace_window = _VECTOR_STAGE_GRACE_PERIOD if gate == "vectorizers" else timeout_floor * 0.25
                if grace_window and phase_state.get("remaining") is not None:
                    phase_state["remaining"] = (phase_state.get("remaining") or 0.0) + grace_window
                phase_state["grace"] = True
            if progressing:
                phase_extensions = int(phase_state.get("extensions", 0) or 0)
                if phase_extensions < _GATE_EXTENSION_LIMIT and phase_state.get("remaining") is not None:
                    base_budget = phase_state.get("budget") or gate_budget or resolved_timeout or timeout_floor
                    phase_extension_budget = max(timeout_floor, base_budget)
                    if (phase_state.get("remaining") or 0.0) < phase_extension_budget:
                        phase_state["remaining"] = phase_extension_budget
                        phase_state["deadline"] = time.perf_counter() + phase_extension_budget
                        phase_state["extensions"] = phase_extensions + 1
                        _PREPARE_PIPELINE_WATCHDOG.setdefault("phase_extensions", []).append(
                            {
                                "stage": stage,
                                "phase": phase_key,
                                "extension_budget": phase_extension_budget,
                                "extensions_used": phase_extensions + 1,
                                "progress_signal": dict(progress_signal),
                            }
                        )
                        if shared_timeout_coordinator is not None:
            ...
            if (
                ...
                if optional_overrun:
                    ...
                    _mark_deferred_gates(
                        ...
                        readiness_ratio=readiness_ratio,
                        vector_heavy=vector_heavy,
                        reason="optional_window_exhausted",
                        stage_budget=phase_state.get("budget") or 0.0,
        ...
        if pending_critical and exhausted_critical >= pending_critical:
            ...
            stop_signaled = stop_event is not None and stop_event.is_set()
            if progress_detected and not stop_signaled:
                ...
                watchdog_telemetry["deferred_recovery_tasks"] = remediation_hints
                _PREPARE_PIPELINE_WATCHDOG.setdefault(
                    "deferred_recovery_tasks", []
                ).append(
                    {
                        "stage": stage,
                        "pending_gates": tuple(sorted(exhausted_critical)),
                        "progress_signal": dict(progress_signal),
                        "remediation_hints": remediation_hints,
                    }
                )
                logger.warning(
                    ...
                    extra={
                        ...
                        "deferred_recovery_tasks": remediation_hints,
                    },
                )
                return
            if stop_signaled:
                # Defer to the explicit stop handler below so cancellation is surfaced
                # even when progress signals are present.
                degraded_payload = watchdog_telemetry.get("degraded_payload") or _mark_deferred_gates(
                    gates=exhausted_critical,
                    stage=stage,
                    pending_gates=readiness_pending,
                    readiness_ratio=readiness_ratio,
                    vector_heavy=vector_heavy,
                    reason="critical_overrun_stop_signal",
                    stage_budget=watchdog_telemetry.get("stage_budget", 0.0),
                    remaining_window=watchdog_telemetry.get("remaining_window", 0.0),
                )
                _PREPARE_PIPELINE_WATCHDOG.setdefault("deferred_recovery_tasks", []).append(
                    {
                        "stage": stage,
                        "pending_gates": tuple(sorted(exhausted_critical)),
                        "progress_signal": dict(progress_signal),
                ...
                _PREPARE_PIPELINE_WATCHDOG.setdefault("degraded", degraded_payload)
                watchdog_telemetry["deferred_recovery_tasks"] = remediation_hints
                watchdog_telemetry["degraded_payload"] = degraded_payload
                watchdog_telemetry["progress_signal"] = dict(progress_signal)
                watchdog_telemetry["stopped"] = True
                _PREPARE_PIPELINE_WATCHDOG["watchdog"] = watchdog_telemetry
            else:
                remediation_hints = render_prepare_pipeline_timeout_hints(
                    vector_heavy,
    ...
    min_stage_timeout = (
        _MIN_STAGE_TIMEOUT_VECTOR if vector_bootstrap_heavy else _MIN_STAGE_TIMEOUT
    )
    ...
    if resolved_wait_timeout is not None:
        ...
        if clamped_wait_timeout > resolved_wait_timeout:
            logger.warning(
                (
                    "prepare_pipeline caller bootstrap wait below minimum; clamping "
                ...
                extra={
                    "requested_timeout": resolved_wait_timeout,
                    "timeout_floor": min_stage_timeout,
                    "effective_timeout": clamped_wait_timeout,
                    "vector_heavy": vector_bootstrap_heavy,
                    "env_menace_bootstrap_wait_secs": env_bootstrap_wait,
                    "env_menace_bootstrap_vector_wait_secs": env_vector_wait,
    ...
    def decorator(cls: type) -> type:
        ...
        def _bootstrap_helpers(
            *,
            ...
            audit_guard = contextlib.nullcontext()
            try:
                from db_router import GLOBAL_ROUTER, bootstrap_audit_buffering
            except Exception:
                pass
            else:
            ...
            try:
                if bootstrap_wait_timeout_value is not bootstrap_wait_timeout_unset:
                    ...
                    if (
                        ...
                        if isinstance(bootstrap_wait_timeout_candidate, int):
                            bootstrap_wait_timeout = float(bootstrap_wait_timeout_candidate)
                        else:
