...
def _load_environment_module(monkeypatch: pytest.MonkeyPatch):
    """Load sandbox_runner.environment with lightweight dependency stubs."""
    ...
    sandbox_runner_pkg.__path__ = [str(repo_root / "sandbox_runner")]
    ...
    spec = importlib.util.spec_from_file_location(
        "sandbox_runner.environment",
        repo_root / "sandbox_runner" / "environment.py",
    )
...
def test_resolver_reports_nested_dependency_import_failure(monkeypatch: pytest.MonkeyPatch):
    env = _load_environment_module(monkeypatch)

    def fake_import_module(name: str):
        if name == "menace.self_debugger_sandbox":
            raise ModuleNotFoundError(
                "No module named 'menace.human_alignment_agent'",
                name="menace.human_alignment_agent",
            )
    ...
    assert "nested dependency import" in message
    assert "menace.human_alignment_agent" in message
    assert "dependency import failure" in message


def test_resolver_returns_packaged_self_debugger_sandbox_class(monkeypatch: pytest.MonkeyPatch):
    env = _load_environment_module(monkeypatch)

    packaged_module = types.ModuleType("menace.self_debugger_sandbox")

    class SelfDebuggerSandbox:
        pass

    packaged_module.SelfDebuggerSandbox = SelfDebuggerSandbox
    ...
    assert resolved is SelfDebuggerSandbox
